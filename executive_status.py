...
def run_system_audit():
    ...
    print(f"  ├── Active Media Files to Copy : {total_files - review_files:,} files ({total_gb:.2f} GB)")
    print(f"  └── S3 Virtual Folder Markers  : {review_files:,} (Zero-byte S3 placeholders - Excluded)")
    print("-" * 72)
    print("Azure Blob Cryptographic MD5 Verification Status:")
    print(f"  ├── Verified in Azure Blob Container : {verified_files:,} files ({verified_gb:.2f} GB) [{pct_complete:.2f}%]")
